    ...
    <nav className="navbar-container bg-white shadow-sm">
      <div className="navbar-inner container mx-auto px-4 flex items-center justify-between">
        <div className="navbar-left flex items-center gap-4">
          <Link href="/" className="navbar-logo-link inline-flex items-center">
        ...
        <div className="navbar-center hidden md:flex items-center gap-6">
          {navItems.map((item, i) => (
            <Link key={i} href={item.href} className="navbar-item text-gray-700 hover:text-gray-900 transition-colors">
              {item.label}
            </Link>
          ))}
        </div>
        ...
        <div className="navbar-right flex items-center gap-4">
          {showCta && (
            <Link href={ctaLink} className="navbar-cta inline-block bg-green-600 hover:bg-green-700 text-white font-medium py-2 px-4 rounded-md transition-colors">
              {ctaLabel}
            </Link>
          )}
